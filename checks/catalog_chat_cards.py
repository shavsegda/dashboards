...
def check_flags_forwarded() -> None:
    ...
    """
    # Параметра `imp` в адресе больше нет (023, FR-007): важность областей жизни
    # спрашивается каждый раз, и решать за страницу нечего.
    h = catalog_render("?u=tg_777&v=9&k=1&tm=0&md=1&p=0")
    ...
    assert "imp" not in f, \
        f"каталог передал флаг важности, которого больше нет: {f['imp']}"
    ok("5. k, tm, md и p доезжают до страницы, флага важности нет")
