...
#
# ПРАВКА 07.08.2026, спека 011. Шесть замеров — Движение, Люди рядом, Восемь
# фактов, Что ещё стоит знать, Деньги за месяц, Области жизни — переехали из
# разговора на свои страницы. Фразы у них больше нет, и требовать её значит
# требовать вернуть путь, который спека отменила. Здесь остались карточки,
# которые правда идут разговором.
PHRASES = {
    "pair_state": "🫂 Как я в наших отношениях",
    "pair_detector": "🌡 Детектор взрыва",
    "pair_context": "👨‍👩‍👧 Про нас",
    "forum_scales": "👥 Встреча форум-группы",
    "labs": "🩸 Анализы",
}

# Те же шесть переехавших: у них обязана быть страница и не должно быть фразы.
MOVED_TO_PAGE = ["state_move", "state_people", "state_facts", "state_note",
...
def check_every_card_has_door() -> None:
    ...
    # Переехавшие открываются страницей, и второго пути к ним нет: два пути к
    # одному замеру на экране — это вопрос «а чем из них правильно».
    for key in MOVED_TO_PAGE:
        assert reg[key]["url"], f"«{key}» переехал на страницу, а адреса нет"
        assert not reg[key]["phrase"], \
            f"у «{key}» осталась фраза для чата — два пути к одному замеру"
    ok("шесть переехавших открываются страницей, фразы у них нет")
