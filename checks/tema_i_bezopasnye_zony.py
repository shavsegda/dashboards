...
def check_theme_vars_with_fallback() -> None:
    """1. Каждый базовый цвет — из темы Телеграма, с запасным своим значением."""
    bad = []
    for rel in PAGES:
        got = root_vars(rel)
        for name, (tg_var, fallback, _dark) in THEME_VARS.items():
            want = f"var({tg_var}, {fallback})"
            if got.get(name, "").replace(" ", "") != want.replace(" ", ""):
                bad.append(f"{rel}: {name} = {got.get(name)!r}, нужно {want!r}")
    assert not bad, "; ".join(bad[:6])
    ok(f"цвета идут из темы Телеграма, своя палитра запасная ({len(PAGES)} страниц)")
