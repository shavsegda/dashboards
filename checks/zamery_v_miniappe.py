...
def check_skip_makes_no_record() -> None:
    ...
    # Свободную строку пропустили кнопкой.
    got = page("state_note", """
  startCard();
  byId('skipStep').click();
  OUT.screen = screen();
""")
    assert got["rows"] == [], "свободную строку пропустили, а запись ушла"
    assert "Пропустил" in visible(got["screen"]), \
        f"после пропуска экран не сказал, что ничего не записано: {visible(got['screen'])[:200]}"
    ok("свободная строка: пропуск не пишет и говорит об этом")
