...
def check_fresh_visible_not_a_task() -> None:
    ...
    #
    # ПРАВКА 10.08.2026, спека 020. Строка в списке была «идёт разговором — нажми,
    # бот начнёт сам». Живой дефект: владелец нажал «Нагрузка по дому», мини-апп
    # закрылся, развернулась клавиатура бота, и он не понял, что произошло.
    # Теперь строка говорит про закрытие экрана и про то, что нужно принести —
    # текст держит `kartochki_bez_stranicy.py`. Здесь сторожим прежнее: подробная
    # подсказка одна, а в списке карточка-разговор всё равно себя объявляет.
    ...
    assert "Замер идёт в переписке с ботом" in visible(rest), \
        "в списке не видно, что замер идёт в переписке с ботом"
